...
// ─── DESIGN TOKENS (Playbook-matched palette) ───
const C = {
  // Backgrounds — deep navy layers
  bg0: "#0B1018",   // deepest (body)
  bg1: "#0F1923",   // page base
  bg2: "#152030",   // page content
  bg3: "#1A2838",   // elevated sections
  bg4: "#1E3045",   // card dark
  bg5: "#243850",   // card hover

  // Cream text hierarchy (matches Playbook ivory/cream typography)
  cream1: "#EDE5D8", // primary headings — warm ivory
  cream2: "#D8D0C4", // body text — warm cream
  cream3: "#BEB6AA", // secondary text
  cream4: "#9A9488", // muted/captions
  cream5: "#706A60", // very muted

  // Cream card backgrounds (matches the light info boxes in Playbook)
  cardCream: "#EDE5D8",
  cardCreamBorder: "#D8CFC0",
  cardCreamText: "#2A2520",
  cardCreamMuted: "#5A5248",

  // Teal/Cyan accents (ActiGraft product line)
  teal: "#5BA8B8",
  tealDark: "#4A96A6",
  tealDeep: "#357888",
  tealBright: "#7CC4D4",
  tealGlow: "rgba(91,168,184,0.08)",

  // Blue accent cards (matches Playbook blue info boxes)
  blue: "#1A3852",
  blueBorder: "#3A7090",
  blueLight: "#2A5070",
...
// ─── STATIC MOLECULAR / HELIX BACKGROUND SVG ───
// Renders a double helix strand with molecular node connections — purely static
const MolecularBg = ({ variant = 0 }) => {
  // Two layout variants so adjacent pages don't look identical
  const helixOffsetX = variant === 0 ? 640 : 120;
  const helixOffsetY = variant === 0 ? 60 : 200;
  const dotSeed = variant * 37; // shifts dot positions
  // Pre-computed static dots — luminous particle field (frozen data aesthetic)
  const dots = [];
  for (let i = 0; i < 65; i++) {
    const seed = (i + dotSeed) * 2654435761;
    const x = ((seed >>> 0) % 1000) / 10;
    const y = ((seed * 31 >>> 0) % 1000) / 10;
    const r = 0.6 + ((seed * 17 >>> 0) % 10) / 10;
    const op = 0.06 + ((seed * 7 >>> 0) % 20) / 100;
    dots.push({ x, y, r, op });
  }
  return (
    <svg viewBox="0 0 820 1100" preserveAspectRatio="xMidYMid slice" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 0 }}>
      <defs>
        <radialGradient id={`glow${variant}`} cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor={C.teal} stopOpacity="0.6" /><stop offset="100%" stopColor={C.teal} stopOpacity="0" /></radialGradient>
      </defs>
      {/* Static luminous dot field */}
      {dots.map((d, i) => (
        <circle key={i} cx={`${d.x}%`} cy={`${d.y}%`} r={d.r} fill={C.teal} opacity={d.op} />
      ))}
      {/* Double helix — two intertwined sine curves with cross-rungs */}
      <g transform={`translate(${helixOffsetX}, ${helixOffsetY})`} opacity="0.045">
        {/* Strand A */}
        ...
        {[0,75,150,225,300,375,450,525,600,675,750,825].map((y, i) => {
          ...
          return <line key={i} x1={x1} y1={y} x2={x2} y2={y} stroke={C.cream4} strokeWidth="1" opacity="0.6" />;
        })}
        {/* Molecular nodes at rung endpoints */}
        {[0,150,300,450,600,750,900].map((y, i) => (
          <g key={i}>
            <circle cx={0} cy={y} r="4" fill={C.teal} opacity="0.5" />
            <circle cx={80} cy={y} r="4" fill={C.teal} opacity="0.5" />
          </g>
        ))}
      </g>
      {/* Scattered molecular nodes with connection lines */}
      <g opacity="0.03">
        {variant === 0 ? <>
          <circle cx="120" cy="180" r="3" fill={C.tealBright} /><circle cx="200" cy="220" r="2.5" fill={C.tealBright} /><line x1="120" y1="180" x2="200" y2="220" stroke={C.tealBright} strokeWidth="0.8" />
          <circle cx="180" cy="800" r="3" fill={C.tealBright} /><circle cx="260" cy="760" r="2" fill={C.tealBright} /><line x1="180" y1="800" x2="260" y2="760" stroke={C.tealBright} strokeWidth="0.8" />
          <circle cx="90" cy="500" r="2.5" fill={C.tealBright} /><circle cx="150" cy="540" r="2" fill={C.tealBright} /><circle cx="50" cy="530" r="1.5" fill={C.tealBright} />
          <line x1="90" y1="500" x2="150" y2="540" stroke={C.tealBright} strokeWidth="0.6" /><line x1="90" y1="500" x2="50" y2="530" stroke={C.tealBright} strokeWidth="0.6" />
        </> : <>
          <circle cx="600" cy="140" r="3" fill={C.tealBright} /><circle cx="680" cy="170" r="2.5" fill={C.tealBright} /><line x1="600" y1="140" x2="680" y2="170" stroke={C.tealBright} strokeWidth="0.8" />
          <circle cx="650" cy="700" r="3" fill={C.tealBright} /><circle cx="720" cy="660" r="2" fill={C.tealBright} /><line x1="650" y1="700" x2="720" y2="660" stroke={C.tealBright} strokeWidth="0.8" />
          <circle cx="580" cy="400" r="2.5" fill={C.tealBright} /><circle cx="640" cy="440" r="2" fill={C.tealBright} /><circle cx="540" cy="430" r="1.5" fill={C.tealBright} />
          <line x1="580" y1="400" x2="640" y2="440" stroke={C.tealBright} strokeWidth="0.6" /><line x1="580" y1="400" x2="540" y2="430" stroke={C.tealBright} strokeWidth="0.6" />
        </>}
      </g>
    </svg>
  );
...
// Cream info card (matches Playbook light cards)
const CreamCard = ({ title, children, borderColor }) => (
  <div className="cream-card" style={{ background: C.cardCream, border: `1px solid ${borderColor || C.cardCreamBorder}`, borderRadius: 6, padding: "16px 18px", marginBottom: 12 }}>
    {title && <div style={{ fontSize: 14, fontWeight: 800, color: C.cardCreamText, marginBottom: 6, fontFamily: "'Georgia', serif" }}>{title}</div>}
    <div style={{ fontSize: 12.5, color: C.cardCreamMuted, lineHeight: 1.65 }}>{children}</div>
  </div>
);

// Dark teal card (matches Playbook blue/teal info boxes)
const TealCard = ({ title, children, icon }) => (
  <div style={{ background: C.blue, border: `1px solid ${C.blueBorder}`, borderRadius: 6, padding: "16px 18px", marginBottom: 12 }}>
